<template>
  <section class="flex overflow-hidden h-[500px] relative">
    ...
    <span
      class="absolute text-4xl right-5 inset-y-0 h-10 m-auto animate-toRight cursor-pointer z-10"
      @click="changImg(1)"
      >➤</span
    >
    <span
      class="absolute text-4xl rotate-180 left-5 inset-y-0 h-10 m-auto animate-toLeft cursor-pointer z-10"
      @click="changImg(-1)"
      >➤</span
    >

    <span
      class="text-4xl h-96 absolute [writing-mode:vertical-rl] font-bold tracking-wider inset-y-0 m-auto bg-gray-700/50 right-14 xl:right-96"
